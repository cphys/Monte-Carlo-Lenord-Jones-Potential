...
listOfLatices = {
    {"square", {{1, 0}, {0, 1}}},
    {"triangle", {{1, 0}, {1/2, Sqrt[3]/2}}},
    {"hexagonal", {{Sqrt[3]/2, 3/ 2}, {-Sqrt[3]/2, 3/2}}}};
...
latticePoints[L_] := 
 latticePoints[L] = 
  DeleteCases[
   DeleteCases[
    Flatten[Table[
      n1[L][[ii]]*a1 + n2[L][[jj]]*a2, {ii, 1, Length[n1[L]]}, {jj, 1, Length[n2[L]]}], 1], {0, 0}], {0., 0.}]
...
aproxfit = 
  fitParameters[1, Aprox[LValue]]*
    Exp[-fitParameters[2, Aprox[LValue]]*L] + fitParameters[3, Aprox[LValue]];
...
elprime1fit = 
  fitParameters[1, ELPrime1]*Exp[-fitParameters[2, ELPrime1]*L] + fitParameters[3, ELPrime1];
...
For[\[Alpha] = .5; list1 = {}; list2 = {}, \[Alpha] < 3, \[Alpha] = \[Alpha] + .01, (
  ...
  latticePoints = 
   DeleteCases[
    DeleteCases[
     Flatten[Table[
       n1[[ii]]*a1 + n2[[jj]]*a2, {ii, 1, Length[n1]}, {jj, 1, Length[n2]}], 1], {0., 0.}], {0, 0}];

  Magnitudes = 
   Table[N[Norm[latticePoints[[vv]]]], {vv, 1, Evaluate[Length[latticePoints]]}];
...
energyVsAlphaFitPlot =
  Show[
   ListPlot[newOne, PlotMarkers -> {pm1, 5}, PlotStyle -> c1, PlotRange -> Full],
   Plot[energyVsAlphaFit, {L, -1, 3}, PlotRange -> Full],
   AxesLabel -> {"\[Alpha]", "V[\[Alpha]]"},
   AspectRatio -> .75,
